...
create procedure dbo.createclientaccesstoken(IN id uuid, IN expiration timestamp without time zone, IN scope text, IN apiclientid integer, IN maxtokencount integer)
    language plpgsql
as
$$
DECLARE
...
BEGIN;
    ...
    INSERT INTO public."DeployJournal" (schemaversionsid, scriptname, applied) VALUES (35, 'Artifacts.PgSql.Structure.Admin.0175-Add-CreateClientAccessToken.sql', now()::timestamp);
    INSERT INTO public."DeployJournal" (schemaversionsid, scriptname, applied) VALUES (36, 'Artifacts.PgSql.Structure.Admin.0176-Update-CreateClientAccessToken.sql', now()::timestamp);

    ALTER SEQUENCE public."DeployJournal_schemaversionsid_seq" RESTART WITH 37;
END;
